...
Panel {
  id: root
  moduleName: "cesarho.xps-power"
  ipcTarget: "cesarho.xps-power"
  ...
  readonly property var chargeLimitOptions: [60, 80, 90, 100]
  // Read straight from sysfs rather than UPower's cached charge-start/end
  // threshold properties, which have been observed to go stale indefinitely
  // (not just lag) after a change made outside UPower's own control path.
  property int activeChargeLimit: -1
  property int activeChargeLimitStart: -1
  readonly property string chargeLimitDisplay: {
    if (root.activeChargeLimit < 0) return "-"
    if (root.activeChargeLimitStart >= 0 && root.activeChargeLimitStart !== root.activeChargeLimit)
      return root.activeChargeLimitStart + "-" + root.activeChargeLimit + "%"
    return root.activeChargeLimit + "%"
  }
  ...
  Process {
    ...
    onExited: function(exitCode) {
      if (exitCode !== 0) root.notifyPrivilegedFailure("Charge limit", exitCode)
      root.refresh()
    }
  }
  ...
  Process {
    ...
    command: [root.binDir + "omarchy-battery-threshold-get"]
    stdout: StdioCollector {
      ...
      onStreamFinished: {
        ...
        if (!isNaN(s)) root.activeChargeLimitStart = s
        if (!isNaN(e)) root.activeChargeLimit = e
      }
    }
  }
  ...
  Process {
    id: chargeModeReadProc
    command: [root.binDir + "omarchy-battery-mode-get"]
    stdout: StdioCollector {
      waitForEnd: true
      onStreamFinished: {
        var mode = text.trim()
        if (mode !== "") root.activeChargeMode = mode
      }
    }
  }
      ...
      Column {
        ...
        Row {
          ...
          Column {
            ...
            InfoPair {
              ...
              value: root.chargeThresholdActive ? root.chargeLimitDisplay : (root.batteryFlowIdle ? "-" : (root.batteryInfo.time || "—"))
            }
            InfoPair {
        ...
        Column {
          ...
          Row {
            ...
            readonly property real cellWidth: root.chargeModeOptions.length > 0
              ? (width - spacing * (root.chargeModeOptions.length - 1)) / root.chargeModeOptions.length
              : 0

            Repeater {
              model: root.chargeModeOptions
              Button {
                required property var modelData
                width: chargeModeRow.cellWidth
                ...
                bordered: true
                active: root.activeChargeMode === modelData
                onClicked: root.setChargeMode(modelData)
              }
            }
          }
        }
          ...
          Row {
            ...
            Repeater {
              ...
              Button {
                ...
                verticalPadding: Style.spacing.controlPaddingY + Style.space(2)
                bordered: true
                active: root.activeChargeLimit === modelData
                enabled: root.activeChargeMode === "Custom"
            ...
            Repeater {
              ...
              Button {
                ...
                onHovered: function(h) {
                  ...
                  } else if (root.cursorActive && root.profileIndex === index) {
